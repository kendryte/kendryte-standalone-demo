...
void mp3_to_wav(const uint8_t *mp3_buf,size_t mp3_size, uint8_t *wav, size_t wav_size)
{
    ...
    int data_bytes = 0, samples;
    uint32_t index = 0;

    //跳过mp3 header
    if (mp3_size > 10 && !strncmp((char *)mp3_buf, "ID3", 3))
    {
        int id3v2size = (((mp3_buf[6] & 0x7f) << 21) | ((mp3_buf[7] & 0x7f) << 14) |
            ((mp3_buf[8] & 0x7f) << 7) | (mp3_buf[9] & 0x7f)) + 10;
    ...
    do
    {
        ...
        samples = mp3dec_decode_frame(&mp3d, mp3_buf, mp3_size, pcm, &info);
        if(data_bytes + samples * 2 * info.channels > wav_size)
            break;
        if (samples)
        {
            memcpy(wav+index, pcm, samples * 2 * info.channels);
            index += samples * 2 * info.channels;
        }
        mp3_buf  += info.frame_bytes;
        mp3_size -= info.frame_bytes;
        data_bytes += samples * 2 * info.channels;
    } while (info.frame_bytes);
    printf("wav size if %d\n", data_bytes);
    //根据结果写入wav文件头
    memcpy(wav, wav_header(info.hz, info.channels, 16, data_bytes), 44);
}
